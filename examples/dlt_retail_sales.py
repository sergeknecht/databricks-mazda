...
def customers():
    return spark.read.csv(
        '/databricks-datasets/retail-org/customers/customers.csv', header=True
    )
...
def sales_orders_cleaned():
    df = dlt.read_stream('sales_orders_raw').join(
        dlt.read('customers'), ['customer_id', 'customer_name'], 'left'
    )
    df = df.withColumn(
        'order_datetime', from_unixtime(df.order_datetime).cast('TIMESTAMP')
    )
...
def sales_order_in_la():
    ...
    df = df.select(
        ...
        df.customer_id,
        df.customer_name,
        explode(df.ordered_products).alias('ordered_products_explode'),
    )
    ...
    dfAgg = df.groupBy(
        ...
        sum(df.ordered_products_explode.qty).alias('quantity'),
    )
