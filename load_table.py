...
dbutils.widgets.text("idx", "")
dbutils.widgets.text("idx_max", "")
...
idx = dbutils.widgets.get("idx")
idx_max = dbutils.widgets.get("idx_max")
catalog_name = dbutils.widgets.get("catalog_name")
schema = dbutils.widgets.get("schema")
table_name = dbutils.widgets.get("table_name")
scope = dbutils.widgets.get("scope")

assert scope, "scope not set"
    ...
    if not table_exists(catalog_name, schema, table_name):
        ...
        for row_pk in df_pk.collect():
            ...
            for curr_sql in sqls:
                print("\t", curr_sql)
                spark.sql(curr_sql)
        return f"created {idx}/{idx_max}: {dbx_qualified_table_name}"
    else:
        return f"SKIPPED {idx}/{idx_max}: {dbx_qualified_table_name}"
...
try:
    ...
    result = perform_task(idx, idx_max, catalog_name, schema, table_name)
