...
def get_auth_token_cxu(
    username: str,
    password: str,
    idcs_url='idcs-b7562c48970d4eed98cde4c77d4b4c34.identity.oraclecloud.com',
    client_id='CXUNITYSI-88f55a2a391c4a9a8a941858255d004c_APPID',
    client_secret='1cf8910a-b6da-468b-93b6-3299baeb202a',
    idcs_primary_audience_url='urn:opc:entitlementid=638686749',
):
    """
    ...
    authentication_token = r['access_token']
...
def call_api_cxu(
    method: str,
    endpoint: str,
    account_url='mmecdp.cxunity.ocs.oraclecloud.com',
    tenant_id='100016',
    access_key='b28b1ed037214ba9ab4814b0d72b2982',
    limit=None,
):
    """
    ...
    if method == 'GET':
        r = requests.get(
            ...
            headers={
                ...
                'x-mcps-tenantId': tenant_id,
                'x-mcps-tenantkey': access_key,
            },
        )
    elif method == 'POST':
        r = requests.post(
...
def cxu_get_segments():
    ...
    response = call_api_cxu(
        method='GET', endpoint='/cxunity/segmentation/v1/segments', limit=500
    )
...
def generate_url_signature_bluekai(
    method: str,
    url: str,
    data=None,
    bkuid='1baf3150d8a57589e4bbbb51d7989fe420c30656b2f7758e59c5e32ccb174a49',
    bksecretkey='af8f4a9979b5f083d34fbfbf64f83f7abc01ee51d318c6659b03bbcb3e6cbee6',
):
    """
...
def bluekai_get_audiences():
    ...
    response = call_api_bluekai(
        method='GET', url='https://services.bluekai.com/Services/WS/audiences'
    )
...
def infinity_extract_data_uc3_4(url, date, use_case, audience):
    ...
    token = create_token_infinity(
        username='jasper.penneman@element61.be', password='zhMJkJUJ^Z2VW@bV'
    )
    ...
    dataframe_spark = (
        spark.createDataFrame(dataframe_pd)
        # add date
        ...
        .withColumn(
            'week',
            F.when((F.col('week') > 50) & (F.col('month') == 1), 0).otherwise(
                F.col('week')
            ),
        )  # correction for beginning of January
        .drop('date_str')
        # add language
        .withColumn(
            'language',
            F.when(
                F.split('Domain', r'\.').getItem(0) != 'www',
                F.split('Domain', r'\.').getItem(0),
            ).otherwise(None),
        )
        # add extension
        .withColumn('split', F.split('Domain', r'\.'))
    ...
    if use_case == 'UC3':
        dataframe_renamed = (
            dataframe_spark.withColumnRenamed('UC3_HeroBanneerSeen', 'HeroBannerSeen')
            .withColumnRenamed('UC3_CTA1_Clicks', 'CTA1_Clicks')
            .withColumnRenamed('UC3_CTA2_Clicks', 'CTA2_Clicks')
        )
    elif use_case == 'UC4':
        dataframe_renamed = (
            dataframe_spark.withColumnRenamed('UC4_USC_Hero_Seen', 'HeroBannerSeen')
            .withColumnRenamed('UC4_USC_CC', 'CTA1_Clicks')
            .withColumnRenamed('UC4_USC_Offer', 'CTA2_Clicks')
        )

    # add use_case and audience
    dataframe_final = dataframe_renamed.withColumn(
        'use_case', F.lit(use_case)
    ).withColumn('audience', F.lit(audience))
...
def add_basic_kpis_uc3_4(df):
    df_new = (
        df.withColumn('cta1_ctr', F.col('CTA1_Clicks') / F.col('Sessions'))
        .withColumn('cta2_ctr', F.col('CTA2_Clicks') / F.col('Sessions'))
        .withColumn(
            'cta_total_ctr',
            (F.col('CTA1_Clicks') + F.col('CTA2_Clicks')) / F.col('Sessions'),
        )
        ...
        .withColumn(
            'configuration_started_conversion_rate',
            F.col('configuration_started') / F.col('Sessions'),
        )
        .withColumn(
            'configuration_completed_conversion_rate',
            F.col('configuration_completed') / F.col('Sessions'),
        )
        .withColumn(
            'tdr_submit_conversion_rate',
            F.col('tdr_submissions') / F.col('Sessions'),
        )
        .withColumn(
            'raq_submit_conversion_rate',
            F.col('raq_submissions') / F.col('Sessions'),
        )
    )
...
def calculate_uplift(df, use_case, audience):
    ...
    if use_case == 'UC3':
        base_df = df.filter(F.col('use_case') == use_case).filter(
            F.col('audience') == audience
        )
    elif use_case == 'UC4':
    ...
    if use_case == 'UC3':
        if audience == 'Audience 1':
            variant_filter = base_df.filter(
                F.col('WebsiteVariant') == 'element1:configstarted'
            )
        elif audience == 'Audience 2':
            variant_filter = base_df.filter(
                F.col('WebsiteVariant') == 'element1:configcompleted'
            )

    elif use_case == 'UC4':
        variant_filter = base_df.filter(
            F.col('WebsiteVariant') == 'element1:highestmodel'
        )

    default = (
        default_filter.select(
            'Domain',
            'date',
            'configuration_started_conversion_rate',
            'configuration_completed_conversion_rate',
        ...
        .withColumnRenamed(
            'configuration_started_conversion_rate',
            'configuration_started_conversion_rate_default',
        )
        .withColumnRenamed(
            'configuration_completed_conversion_rate',
            'configuration_completed_conversion_rate_default',
        )
        .withColumnRenamed(
            'tdr_submit_conversion_rate', 'tdr_submit_conversion_rate_default'
        )
        .withColumnRenamed(
            'raq_submit_conversion_rate', 'raq_submit_conversion_rate_default'
        )
    )
    ...
    variant = (
        ...
        .withColumnRenamed(
            'configuration_started_conversion_rate',
            'configuration_started_conversion_rate_variant',
        )
        .withColumnRenamed(
            'configuration_completed_conversion_rate',
            'configuration_completed_conversion_rate_variant',
        )
        .withColumnRenamed(
            'tdr_submit_conversion_rate', 'tdr_submit_conversion_rate_variant'
        )
        .withColumnRenamed(
            'raq_submit_conversion_rate', 'raq_submit_conversion_rate_variant'
        )
    )
    ...
    uplift = (
        join_default_variant.withColumn(
            'configuration_started_uplift',
            (
                F.col('configuration_started_conversion_rate_variant')
                - F.col('configuration_started_conversion_rate_default')
            )
            / F.col('configuration_started_conversion_rate_default'),
        )
        .withColumn(
            'configuration_completed_uplift',
            (
                F.col('configuration_completed_conversion_rate_variant')
                - F.col('configuration_completed_conversion_rate_default')
            )
            / F.col('configuration_completed_conversion_rate_default'),
        )
        .withColumn(
            'tdr_submit_uplift',
            (
                F.col('tdr_submit_conversion_rate_variant')
                - F.col('tdr_submit_conversion_rate_default')
            )
            / F.col('tdr_submit_conversion_rate_default'),
        )
        .withColumn(
            'raq_submit_uplift',
            (
                F.col('raq_submit_conversion_rate_variant')
                - F.col('raq_submit_conversion_rate_default')
            )
            / F.col('raq_submit_conversion_rate_default'),
        )
    )
    ...
    uplift_final = uplift.withColumn('use_case', F.lit(use_case)).withColumn(
        'audience', F.lit(audience)
    )
...
def infinity_extract_data_uc5(url, date):
    ...
    final_df = (
        ...
        .withColumn(
            'nsc',
            F.split(dataframe_spark['ELQ_Campaign_Name'], '_').getItem(0),
        )
        .withColumn(
            'audience',
            F.split(dataframe_spark['ELQ_Campaign_Name'], '_').getItem(5),
        )
    )
...
def add_basic_kpis_uc5(df):
    df_new = (
        df.withColumn(
            'unique_ctr',
            F.col('Unique_Email_Click') / F.col('Unique_Email_Send'),
        )
        .withColumn(
            'unique_open_rate',
            F.col('Unique_Email_Open') / F.col('Unique_Email_Send'),
        )
        .withColumn(
            'unique_click_to_open_rate',
            F.col('Unique_Email_Click') / F.col('Unique_Email_Open'),
        )
        .withColumn('unique_configuration_started', F.col('Unique_Configurator'))
        .withColumn(
            'unique_configuration_completed',
            F.col('Unique_Configurator_Completed'),
        )
...
def maxymiser_extract_data_uc3_4(use_case, filepath):
    ...
    clean_maxymiser_base = (
        ...
        .withColumn(
            'date',
            to_date(to_timestamp('Generation Time and Date', 'MM/dd/yyyy HH:mm:ss')),
        )
    ...
    if use_case == 'UC3':
        clean_maxymiser_final = (
            clean_maxymiser_base.withColumn(
                'audience',
                F.when(
                    col('Segmentation Rule') == 'Unsegmented Traffic',
                    F.lit('Audience 1'),
                ).when(col('Segmentation Rule') == 'Rule1', F.lit('Audience 2')),
            )
            .withColumn(
                'WebsiteVariant',
                F.when(
                    col('Element: Element1') == 'Default',
                    F.lit('element1:Default'),
                )
                .when(
                    col('Element: Element1') == 'ConfigStarted',
                    F.lit('element1:configstarted'),
                )
                .when(
                    col('Element: Element1') == 'ConfigCompleted',
                    F.lit('element1:configcompleted'),
                ),
            )
            ...
            .withColumn(
                'NoAction',
                F.when(col('NoAction') > 1, 1).otherwise(col('NoAction')),
            )
            .withColumn(
                'ConfiguratorComplet',
                F.when(col('ConfiguratorComplet') > 1, 1).otherwise(
                    col('ConfiguratorComplet')
                ),
            )
            .withColumn(
                'ConfiguratorStarted',
                F.when(col('ConfiguratorStarted') > 1, 1).otherwise(
                    col('ConfiguratorStarted')
                ),
            )
            .withColumn(
                'Global_RAQ',
                F.when(col('Global_RAQ') > 1, 1).otherwise(col('Global_RAQ')),
            )
            .withColumn(
                'Global_TDR',
                F.when(col('Global_TDR') > 1, 1).otherwise(col('Global_TDR')),
            )
            .withColumn(
                'UC3_USC_Hero_Seen',
                F.when(col('UC3_USC_Hero_Seen') > 1, 1).otherwise(
                    col('UC3_USC_Hero_Seen')
                ),
            )
            .withColumn(
                'UC3_USC_cta1',
                F.when(col('UC3_USC_cta1') > 1, 1).otherwise(col('UC3_USC_cta1')),
            )
            .withColumn(
                'UC3_USC_cta2',
                F.when(col('UC3_USC_cta2') > 1, 1).otherwise(col('UC3_USC_cta2')),
            )
        )

    elif use_case == 'UC4':
        clean_maxymiser_final = (
            clean_maxymiser_base.withColumn('audience', lit(None))
            .withColumn(
                'WebsiteVariant',
                F.when(
                    col('Element: Element1') == 'Default',
                    ...
                    col('Element: Element1') == 'highestModel',
                    F.lit('element1:highestmodel'),
                ),
            )
            .drop('Segmentation Rule', 'Element: Element1')
            .fillna(
            ...
            .withColumn(
                'UC4_USC_CC',
                F.when(col('UC4_USC_CC') > 1, 1).otherwise(col('UC4_USC_CC')),
            )
            .withColumn(
                'UC4_USC_Hero_Seen',
                F.when(col('UC4_USC_Hero_Seen') > 1, 1).otherwise(
                    col('UC4_USC_Hero_Seen')
                ),
            )
            .withColumn(
                'UC4_USC_Offer',
                F.when(col('UC4_USC_Offer') > 1, 1).otherwise(col('UC4_USC_Offer')),
            )
        )
