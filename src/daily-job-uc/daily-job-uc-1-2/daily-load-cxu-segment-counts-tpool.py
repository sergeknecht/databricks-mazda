...
segments_df = pd.DataFrame.from_records(response['items'])[
    ['name', 'uniqueId']
...
for index, row in segments_df.iterrows():
    ...
    try:
        if (name_split[1] in ('UC1', 'UC2')) and row['name'] not in (
            'MME_UC1_A2_Suppression_Deceased',
            'MAG_UC1_A1_Suppression_ExistingPrivateCustomers_withBKUUID',
            'MMD_UC1_A1_Suppression_ExistingPrivateCustomers_BKUUID',
            'MME_UC2_A1_Suppression_Leads_60days',
            'MMB_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMB_UC2_A1_FB_Suppression_Leads',
            'MMB-LUX_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMB-LUX_UC2_A1_FB_Suppression_Leads',
            'MMCR_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMCR_UC2_A1_FB_Suppression_Leads',
            'MMDK_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMDK_UC2_A1_FB_Suppression_Leads',
            'MMIRL_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMIRL_UC2_A1_FB_Suppression_Leads',
            'MMNL_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMNL_UC2_A1_FB_Suppression_Leads',
            'MMP_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMP_UC2_A1_FB_Suppression_Leads',
            'MMRO_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMRO_UC2_A1_FB_Suppression_Leads',
            'MMSK_UC1_A1_FB_Suppression_ExistingPrivateCustomers',
            'MMSK_UC2_A1_FB_Suppression_Leads',
        ):
...
def task(identifier, segment_id):
    ...
    return (
        identifier,
        count,
    )  # count cn be real number or -1 (still processing count)
...
task_params = [
    ...
    for segment_id in segments_df[segments_df['segment_count_cxu'] < 0]['uniqueId']
]
...
segment_counts_df = pd.DataFrame.from_records(
    result_counts, columns=['uniqueId', 'segment_count_cxu']
)  # convert JSON response to table
...
if not segment_counts_df.empty:
    test = segments_df.set_index('uniqueId').join(
        segment_counts_df.set_index('uniqueId'), rsuffix='_new'
    )
    test['segment_count_cxu'] = test['segment_count_cxu_new'].combine_first(
        test['segment_count_cxu']
    )
...
task_params = [
    (segment_id, segment_id)
    for segment_id in test[test['segment_count_cxu'] < 0]['uniqueId']
]
...
spark_df.write.format('delta').mode('overwrite').saveAsTable(
    'element61.uc_reporting.cxu_segment_counts'
)
...
for batch in batches:
    ...
    if bad_segments_round_2:  # len(bad_segments_round_2) > 0:
...
new_counts_pd = pd.DataFrame(
    new_counts,
    columns=['name', 'segment_id_cxu', 'nsc', 'use_case', 'segment_count_cxu'],
)
...
try:
    ...
    success = new_counts_df.filter(
        (F.col('segment_count_cxu').isNotNull()) & (F.col('segment_count_cxu') != -1)
    ).withColumn('status', F.lit('success - count was renewed'))

    # FAIL: if count is -1 or null, then use historic data to find a 'good' count

    # get historic data with 'good' count
    historic_cxu_segment_counts_good = spark.sql(
        """select * from uc_reporting.cxu_segment_counts where segment_count_cxu is not null and segment_count_cxu != -1"""
    )
    ...
    minus_segments_good = historic_cxu_segment_counts_good.filter(
        F.col('segment_id_cxu').isin(minus_segment_ids)
    )
    minus_segments_good_date = minus_segments_good.groupBy('name').agg(
        F.max('date').alias('date')
    )

    # fix with historic data
    minus_segments_fixed = (
        historic_cxu_segment_counts_good.join(minus_segments_good_date, ['name', 'date'])
        .select('name', 'segment_id_cxu', 'nsc', 'use_case', 'segment_count_cxu')
        .withColumn('status', F.lit('count was -1, but fixed via historic data'))
    )
    minus_segments_fixed_ids = [
        data[0] for data in minus_segments_fixed.select('segment_id_cxu').collect()
    ]

    # if not fixed with historic data: keep the -1
    minus_segments_not_fixed = minus.filter(
        ~F.col('segment_id_cxu').isin(minus_segments_fixed_ids)
    ).withColumn(
        'status',
        F.lit('count was -1 and could not be fixed via historic data'),
    )
    ...
    # get historic 'good' counts for these segments and find most recent date
    null_segments_good = historic_cxu_segment_counts_good.filter(
        F.col('segment_id_cxu').isin(null_segment_ids)
    )
    null_segments_good_date = null_segments_good.groupBy('name').agg(
        F.max('date').alias('date')
    )

    # fix with historic data
    null_segments_fixed = (
        historic_cxu_segment_counts_good.join(null_segments_good_date, ['name', 'date'])
        .select('name', 'segment_id_cxu', 'nsc', 'use_case', 'segment_count_cxu')
        .withColumn('status', F.lit('count was null, but fixed via historic data'))
    )
    null_segments_fixed_ids = [
        data[0] for data in null_segments_fixed.select('segment_id_cxu').collect()
    ]

    # if not fixed with historic data: keep the null
    null_segments_not_fixed = null.filter(
        ~F.col('segment_id_cxu').isin(null_segments_fixed_ids)
    ).withColumn(
        'status',
        F.lit('count was null and could not be fixed via historic data'),
    )

    # concatenate new counts again
    new_counts_concat = (
        success.unionByName(minus_segments_fixed)
...
new_counts_final = (
    new_counts_concat.withColumn('date_str', F.lit(today_date))
    ...
    .withColumn(
        'week',
        F.when((F.col('week') > 50) & (F.col('month') == 1), 0).otherwise(F.col('week')),
...
new_counts_final.write.mode('append').format('delta').save(
    '/uc_reporting/uc_1_2/cxu_segment_counts'
)
