...
tbcoc_product = spark.read.format('csv').option('inferSchema','true').option('header','true').load("s3://mazda-bi20-data-nonprd/data/Bronze/tbcoc_product.csv")
#display(tbcoc_product)
tbcoc_product.write.format("delta").mode("overwrite").saveAsTable("bronze.lztbcoc_product")
...
LZTBCOC_VALUE_modified = LZTBCOC_VALUE.withColumn('DISTRIBUTOR',col('DISTRIBUTOR').cast(IntegerType())) \
...
LZTBCOC_VALUE_modified.write.format("delta").mode("overwrite").saveAsTable("bronze.LZTBCOC_VALUE")
...
LZBTNCOCV = spark.read.format("csv").option("inforeSchema", "true").option("header", "true").load(path)
