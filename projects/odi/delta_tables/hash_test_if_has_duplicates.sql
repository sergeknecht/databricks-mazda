-- Example script: test if on full table we get duplicate SHA's
WITH table1 AS (
    SELECT
        *
    FROM
...
HAVING
    COUNT(*) > 1;
-- calculation hash_data done with SHA512: 574 seconds
-- calculation hash_data done with MD5:    499 seconds (hash_pk = 2 secs)
-- Note: MD5 requires less space and is fast algorithm.
-- Note: no duplicates !
-- Note: this is the widest table. So other tables will all go much faster.
-- Note: with a partition swap the hash should only be calculated on the partition in question
