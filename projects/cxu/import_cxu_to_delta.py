...
df = spark.createDataFrame(
    [file for file in files if "__" in file.path and file.path.endswith(".csv.gz")]
)
...
df = df.withColumn(
    "file_dt", F.from_unixtime(col("modificationTime") / 1000)
)  # F.udf(lambda x: get_dt(x))("modificationTime"))
df = df.alias("df")
df.show()
...
df_lf = (
    df.groupby("data_type")
    .agg(F.max("modificationTime").alias("modificationTime"))
    .alias("df_lf")
)
# df_lf = df_lf.filter(F.col('modificationTime') > 1709901250000)
df_not_process = df.join(
    df_lf,
...
for row in df_todo.collect():
    ...
    df_data.write.format("delta").mode("overwrite").option(
...
df_files = spark.createDataFrame(files).withColumn(
    "file_dt", F.from_unixtime(col("modificationTime") / 1000)
)
...
df = (
    ...
    .option(
        "cloudFiles.schemaLocation", f"/mnt/bronze/cxu/mle_bi_lz/schema/{data_type_db}"
    )
    .option("mergeSchema", "true")
    .option(
        "modifiedAfter", "2022-10-15 11:34:00.000000 UTC-3"
    )  # To ingest files that have a modification timestamp after the provided timestamp.
